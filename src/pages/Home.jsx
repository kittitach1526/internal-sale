...
// ข้อมูลจำลองสำหรับกราฟ (สามารถเปลี่ยนค่าตรงนี้ได้)
const chartData = [
    ...
    <div className="relative group isolate p-8 bg-blue-600/5 backdrop-blur-md border border-white/10 rounded-[2rem] shadow-2xl [clip-path:inset(0_round_2rem)] transition-all duration-300 hover:border-white/20">
      ...
      <div className="relative z-10">
        ...
        <div className="mt-4 flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold ${isPositive ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}>
            {isPositive ? "↑" : "↓"} {trend}
          </span>
          <span className="text-slate-600 text-[10px] font-medium uppercase tracking-tighter">จากเดือนก่อน</span>
        </div>
      </div>
    </div>
  );
...
function ChartCard({ title, income, expense, color = "#3b82f6" }) {
  return (
    <div className="relative group isolate bg-white/[0.03] border border-white/5 rounded-[2rem] p-6 backdrop-blur-md shadow-2xl [clip-path:inset(0_round_2rem)] transition-all hover:border-white/10">
      ...
      {/* Header: รายรับ - รายจ่าย */}
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div>
          <h3 className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-3">{title}</h3>
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full shadow-[0_0_8px_rgba(255,255,255,0.8)]" style={{ backgroundColor: color }}></div>
        ...
        <div className="text-[10px] bg-white/5 border border-white/10 text-slate-500 px-2 py-1 rounded-lg">Real-time</div>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-8 animate-in fade-in duration-1000">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-black text-white tracking-tight uppercase">Dashboard Insight</h1>
        <p className="text-slate-500 mt-1 font-medium text-sm">ยินดีต้อนรับกลับมา, <span className="text-blue-400">คุณสมชาย</span></p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="ออเดอร์ทั้งหมด" value="456" trend="2.4%" isPositive={false} />
        <StatCard title="กำไรสุทธิ" value="฿ 124,000" trend="14.1%" />
      </div>

      {/* 3 Chart Cards Row (ที่ปรับปรุงใหม่) */}
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 backdrop-blur-sm transition-all hover:bg-white/[0.04]">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-white tracking-tight">รายการล่าสุด</h3>
          <button className="text-xs text-blue-500 font-bold hover:underline">ดูทั้งหมด</button>
        </div>
        <div className="space-y-3">
          {[1, 2, 3].map(i => (
            <div key={i} className="group flex justify-between items-center p-4 bg-white/[0.03] rounded-2xl border border-white/5 hover:border-white/10 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-white">รายการสั่งซื้อ #INV-00{i}</p>
                  <p className="text-[10px] text-slate-500 uppercase mt-0.5">25 เมษายน 2026 • 14:30</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm font-black text-white">฿ 4,500.00</p>
                <p className="text-[10px] text-green-400 font-bold uppercase tracking-tighter">สำเร็จแล้ว</p>
              </div>
            </div>
          ))}
        </div>
